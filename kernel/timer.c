...
void timer_handler(struct regs *r){
	char str1[3], str2[3], str3[3], *hourStr,*minuteStr,*secondStr;
	int temphours;
	timer_ticks++;
	if(timer_ticks % 18 == 0){
		...
		hourStr = itos(str1, temphours);
		minuteStr = itos(str2, minutes);
		secondStr = itos(str3, seconds);
		putstimer(hourStr,":",minuteStr,":",secondStr,ampm);
		seconds++;
	}
